...
public final class ThreadFactoryBuilder implements Builder<ThreadFactory> {
    ...
    private static ThreadFactory build(ThreadFactoryBuilder builder){
        ...
        final String nameFactory = builder.namePrefix;
        final Boolean daemon = builder.daemon;
        final Integer priority = builder.priority;
        final Thread.UncaughtExceptionHandler handler = builder.uncaughtExceptionHandler;
        final AtomicLong count = (null == nameFactory) ? null : new AtomicLong();
        return r -> {
            final Thread thread = backingThreadFactory.newThread(r);
            if(null != nameFactory){
                thread.setName(nameFactory + "_" + count.getAndIncrement());
            }
            if(null != daemon){
    ...
    public ThreadFactoryBuilder threadFactoryBuilder(ThreadFactory backingThreadFactory){
